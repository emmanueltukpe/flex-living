...
export class GoogleController extends BaseController {
  ...
  getReviews = this.asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (result.status === 'success') {
      ...
      const statusCode = result.message?.includes('Place ID is required') ? 400 : 500;
      this.sendError(res, result.message || 'Failed to fetch Google reviews', statusCode);
    }
  });

  searchPlaces = this.asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    if (result.status === 'success') {
      ...
      this.sendError(res, result.message || 'Failed to search places', statusCode);
    }
  });
